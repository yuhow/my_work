...
modeified version based on Tensorflow version:
...
def visualize_boxes_and_labels_on_image_array(image,
    ...
    if output_text != None:
        file_txt = open(output_text, 'a+')
        file_txt.write(image_name+' ')
        sys.stdout.write(image_name + ' is under processing...')
        sys.stdout.flush()
    ...
    for box, color in box_to_color_map.items():
        ...
        for key in category_index.keys():
            if category_index[key]['name'] == box_to_display_str_map[box][0].strip().split(':')[0]:
                object_id = category_index[key]['id']
        
        file_txt.write('%.2f %.2f %.2f %.2f %s ' % (xmin*image.shape[0], ymin*image.shape[1],\
                                                    xmax*image.shape[0], ymax*image.shape[1],\
                                                    object_id))
        #print xmin*image.shape[0], ymin*image.shape[1], \
        #      xmax*image.shape[0], ymax*image.shape[1], box_to_display_str_map[box]
