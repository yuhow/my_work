...
#   3   ->   10    ->   5     ->   1
# add hidden layer (1 in and 10 out)
l1 = add_layer(xs, 3, 5, activation_function=tf.nn.softplus)
#l1 = add_layer(xs, 3, 10, activation_function=tf.nn.relu)

# add hidden layer (10 in and 10 out)
l2 = add_layer(l1, 5, L2_NEURON, activation_function=tf.nn.softplus)
#l2 = add_layer(l1, 10, 5, activation_function=tf.nn.relu)

# add output layer (10 in and 1 out)
prediction = add_layer(l2, L2_NEURON, 1, activation_function=None)
...
ax_l2 = []
for k_idx in range(L2_NEURON):
    ax_l2.append(fig.add_subplot(5, 3, k_idx*3+2))
...
for i in range(10000):
    ...
    if i % 50 == 0:
        ...
        l2_kernels = sess.run(l2, feed_dict={xs: input_template})
        prediction_value = sess.run(prediction, feed_dict={xs: input_template})

        # l2 output
        l2_output = []
        for k_idx in range(l2_kernels.shape[1]):
            if i == 0:
                plot_tmp, = ax_l2[k_idx].plot(x_data, l2_kernels[:, k_idx], c='blue', label='kernel_2-'+str(k_idx+1))
            else:
                plot_tmp, = ax_l2[k_idx].plot(x_data, l2_kernels[:, k_idx], c='blue')
            ax_l2[k_idx].legend(loc="upper right", prop={'size': 10})
            l2_output.append(plot_tmp)
        ...
        ax.set_ylim((np.amin(y_data[0])-1, np.amax(y_data[0])+1))
        plot_output, = ax.plot(x_data, prediction_value, 'r.')
        l2_output.append(plot_output)
        frame_seq.append(tuple(l2_output))
