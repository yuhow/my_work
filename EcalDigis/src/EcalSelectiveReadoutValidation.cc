...
//   }
  ...
  meDccIntegral_ = book1D("hDccIntegral",
			  "DCC event size: probability to have a DCC event larger than a given size"
			  "Nevts", 1000, 0., 100.);
...
/*
  ...
  es.get< L1GtPrescaleFactorsAlgoTrigRcd>().get(l1GtPfAlgo);
  ...
  int bitNum = 0;
  for(vector<int>::iterator it = prescaleFactorsAlgoTrig.begin();
      it != prescaleFactorsAlgoTrig.end();
      it++) {

      cout<<"Bit number "<<bitNum<<"'s prescaling factor = "<<*it<<endl;
      bitNum++;
  }
*/
  edm::Handle<reco::VertexCollection> primaryVertexHandle;
  event.getByLabel("offlinePrimaryVertices",primaryVertexHandle);
...
void EcalSelectiveReadoutValidation::analyzeDataVolume(const Event& e,
  ...
  for(unsigned iDcc0 = 0; iDcc0 <  nDccs_; ++iDcc0){
    //fill(meDccIntegral_, getDccEventSize(iDcc0, nPerDcc_[iDcc0])/kByte_);
    //cout<<getDccEventSize(iDcc0, nPerDcc_[iDcc0])/kByte_<<endl;
    //total_Dcc += getDccEventSize(iDcc0, nPerDcc_[iDcc0])/kByte_;
    total_Dcc += getDccSrDependentPayload(iDcc0, nLiRuPerDcc_[iDcc0],nLiPerDcc_[iDcc0])/kByte_+getDccSrDependentPayload(iDcc0, nHiRuPerDcc_[iDcc0],nHiPerDcc_[iDcc0])/kByte_;
