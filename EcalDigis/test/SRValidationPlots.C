...
void SRValidationPlots(TString inputfile = "srvalid_hists_QCD_Pt_30_50_13TeV_PVtx_L1_all_TH_2_4_v2.root",
                       const char* ext = ".root",
                       int mode_ = 1111){
...
struct Plot{
  ...
  void run(TString inputfile = "EcalSelectiveReadoutValidationHistos.root",
    ...
    barrel = digit(mode, 1) || digit(mode, 2);  //1000*eep+100*ebp+10*ebm+eem

    endcap = digit(mode, 0) || digit(mode, 3);  //1000*eep+100*ebp+10*ebm+eem
