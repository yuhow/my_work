...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        ...
        if current_saving < current_bribe:
            #days_to_next_bribe = math.ceil((current_bribe - current_saving) / float(current_salary))
            days_to_next_bribe = 10
        else:
            days_to_next_bribe = 0
        ...
        current_saving += current_salary * days_to_next_bribe - current_bribe
        total_earned += current_salary * days_to_next_bribe
