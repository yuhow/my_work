...
public :
      ...
      for ( unsigned int iregion(0); iregion<3; ++iregion )
        {
          ...
          h_dN_vs_dphi_A [iregion]->Delete();
          h_dpT_vs_dphi_A[iregion]->Delete();
  ...
  TH1D*  GetTH1D_dN_vs_dphi_A( unsigned int iregion )  { return h_dN_vs_dphi_A[iregion];              }
  TH1D*  GetTH1D_dpT_vs_dphi_A( unsigned int iregion ) { return h_dpT_vs_dphi_A[iregion];             }
  ///
  ...
  void SetTH1D_dN_vs_dphi_A ( unsigned int iregion, const TH1D* TH1D_dN_vs_dphi_A  ) { h_dN_vs_dphi_A [iregion]->Add(TH1D_dN_vs_dphi_A);  }
  void SetTH1D_dpT_vs_dphi_A( unsigned int iregion, const TH1D* TH1D_dpT_vs_dphi_A ) { h_dpT_vs_dphi_A[iregion]->Add(TH1D_dpT_vs_dphi_A); } 
  //new
...
public :
  ...
  ~UEActivityFinder() 
      ...
      for ( unsigned int iregion(0); iregion<3; ++iregion )
	{
	  ...
	  _h_dN_vs_dphi_A [iregion]->Delete();
	  _h_dpT_vs_dphi_A[iregion]->Delete();
  ...
  Bool_t findMC( TClonesArray&, TClonesArray&, UEActivity&);
  Bool_t findDATA( TClonesArray&, TClonesArray&, UEActivity& ,TrackinJet& );
private :
  ...
  TH1D** _h_dpT_vs_dphi;
  //new
  TH1D** _h_dN_vs_dphi_A;
  TH1D** _h_dpT_vs_dphi_A;
...
public :
  ...
  void fillGen( UEActivity& );
  void fillReco( UEActivity& );
